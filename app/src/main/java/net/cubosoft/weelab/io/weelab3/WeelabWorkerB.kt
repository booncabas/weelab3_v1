...
class WeelabWorkerB(context: Context, userParameters: WorkerParameters) : Worker(context, userParameters) {
    ...
    override fun doWork(): Result {
        try {
            ...
            Log.i(WEELAB_TAG, hour2.toString() + ":" + minute2.toString() + "_BBB_WORK_STARTED in ${Thread.currentThread().id} ${Thread.currentThread().name}")
            ///////////////////////////////////////////////////
            ...
            for (i in 0 .. 9){
                ...
                Log.w(WEELAB_TAG,hour.toString() + ":" + minute.toString() + "_BBB_loop_" + i + "_***************->" + hour.toString() + ":" + minute.toString())
                if( i == 0){
                    allTs += hour.toString() + ":" + minute.toString() + "_BBB_start_looooooooop*"
                }
                try {
                    if (!objSocket.isSocketOpen()){
                        objSocket.initWebSocket()
                        Log.w(WEELAB_TAG, hour.toString() + ":" + minute.toString() + "_BBB_SOCKET_STARTED_in_try")
                        allTs += hour.toString() + ":" + minute.toString() + "_BBB_started_in_try*"
                    } else {
                        allTs += hour.toString() + ":" + minute.toString() + "_BBB_already_open*"
                    }
                }catch (e: Exception){
                    objSocket.initWebSocket()
                    Log.w(WEELAB_TAG, hour.toString() + ":" + minute.toString() + "_BBB_SOCKET_STARTED_in_catch")
                    allTs += hour.toString() + ":" + minute.toString() + "_BBB_started_in_catch*"
                }
                ...
                if(myObjCount.getIdThreadToClose() == Thread.currentThread().id.toString()){
                    ...
                    Log.w(WEELAB_TAG,hour.toString() + ":" + minute.toString() + "_BBB_End_in_${Thread.currentThread().id}")
                    Log.w(WEELAB_TAG,hour.toString() + ":" + minute.toString() + "_BBB_End_work_terminated_rejected_SUCCESS_${Thread.currentThread().id}")
                    return  Result.success()
                }
                if(idLastThread != Thread.currentThread().id.toString()){
                    myObjCount.setIdThreadToClose(idLastThread)
                }
                Log.w(WEELAB_TAG,hour.toString() + ":" + minute.toString() + "_BBB_for_" + i + "_in ${Thread.currentThread().id} _counter_"+myObjCount.getCounter())
//                Log.w(WEELAB_TAG,hour.toString() + ":" + minute.toString() + "_BBB_CCCCounter_add_in_name_${Thread.currentThread().name}_id_${Thread.currentThread().id}")
                myObjCount.setCounterAndName(Thread.currentThread().id.toString())
            }
            Log.w(WEELAB_TAG,hour2.toString() + ":" + minute2.toString() + "_BBB_Result_work_completed_SUCCESS_in ${Thread.currentThread().id} _counter_"+myObjCount.getCounter())
            return Result.success()
        } catch (e: Exception) {
            Log.w(WEELAB_TAG,"_BBB_Result_work_incomplete_RETRY_in ${Thread.currentThread().id} _counter_"+myObjCount.getCounter())
            return Result.retry()
        }
    }
