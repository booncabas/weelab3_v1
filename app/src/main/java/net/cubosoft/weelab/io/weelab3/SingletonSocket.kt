...
class SocketManager private constructor(private var context: Context) {
    ...
    private var socketAliveCounter: Int = 0
...
//            webSocketClient.setSocketFactory(socketFactory)
    ...
    fun updateCounter() {
        socketAliveCounter += 1
    }

    fun getCounter(): Int {
        return socketAliveCounter
    }

    fun startLoop(){
        val prefs: SharedPreferences = context.getSharedPreferences(SHARED_PREF_FILE_NAME, 0)
        var allTs = prefs.getString(TEST_TS, "")
        val prefEditor = prefs.edit()
        for (i in 0 .. 10000) { // minutes
            updateCounter()
            /////////////////////////////////////////////////
            val c = Calendar.getInstance()
            val hour = c.get(Calendar.HOUR_OF_DAY)
            val minute = c.get(Calendar.MINUTE)
            Log.w(WEELAB_TAG,"_socket_loop_" + i + "_***************->" + hour.toString() + ":" + minute.toString())
            allTs += "$hour:$minute "
            if( i == 0){
                allTs += "start_socket_looooooooop_aux"
            }
            allTs = "$allTs-$hour:$minute "
            /////////////////////////////////////////////////
            try {
                if (!isSocketOpen()) {
                    initWebSocket()
                    allTs += "started_in_try*"
                }
                else{
                    allTs += "already_open*"
                }
                ...
                initWebSocket()
                allTs += "started_in_catch*"
            }
            prefEditor.putString(TEST_TS, allTs)
            prefEditor.apply()
            Thread.sleep(60000)//
        }
    }
